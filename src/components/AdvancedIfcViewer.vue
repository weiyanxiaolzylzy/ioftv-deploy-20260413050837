...
export default {
  ...
  computed: {
    ...
    iframeSrc() {
      ...
      const base = this.getIframeViewerBase();
  ...
  methods: {
    ...
    onIframeMessage(event) {
      // 忽略来自非同源 iframe 的消息
      const allowedOrigins = [this.getIframeTargetOrigin()];
    ...
    postToIframe(msg) {
      ...
      const targetOrigin = this.getIframeTargetOrigin();
      iframe.contentWindow.postMessage(msg, targetOrigin);
    },
    getIframeViewerBase() {
      const raw = process.env.VUE_APP_IFC_URL || `${window.location.origin}/ifc`;
      return raw.replace(/\/+$/, '');
    },
    getIframeTargetOrigin() {
      return new URL(this.getIframeViewerBase(), window.location.origin).origin;
    },
