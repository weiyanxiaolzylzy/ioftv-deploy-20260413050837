...
function syncBanzuPhotoMappings() {
    try {
        const groupsRes = dbApi.data.getAllGroups();
        ...
        for (const group of groups) {
            ...
            if (mapped && group.photo_url !== mapped) {
                dbApi.data.updateGroup(group.id, { photo_url: mapped });
            }
        }
        const starRes = dbApi.data.getStar();
...
// 更新标兵信息
app.post('/api/star', requireRole(['admin', 'operator']), banzuUploadMiddleware, (req, res) => {
    try {
        const currentStarRes = dbApi.data.getStar();
        ...
        const rankingRes = dbApi.data.getRankings(period);
        ...
        dbApi.data.updateRankingsByPeriod(period, ranking);
...
app.get('/api/ranking', (req, res) => {
    try {
        const rankingRes = dbApi.data.getRankings();
        const workshopRateRes = dbApi.data.getSetting('workshopFirstPassRate');
...
app.post('/api/ranking', requireRole(['admin', 'operator']), (req, res) => {
    try {
        ...
        dbApi.data.updateRankingsByPeriod(period, normalized);
        if (req.body && req.body.workshopFirstPassRate != null) {
            const rate = Number(req.body.workshopFirstPassRate);
            if (!Number.isNaN(rate)) {
                dbApi.data.setSetting('workshopFirstPassRate', String(rate), '车间一次通过率');
            }
        }
...
app.get('/api/groups', (req, res) => {
    try {
        const result = dbApi.data.getAllGroups();
...
// 添加班组
app.post('/api/groups', requireRole(['admin', 'operator']), banzuUploadMiddleware, (req, res) => {
    try {
        ...
        const createRes = dbApi.data.createGroup(newGroup);
        if (!createRes.success) throw new Error(createRes.error || '新增班组失败');
        const listRes = dbApi.data.getAllGroups();
...
app.delete('/api/groups/:id', requireRole(['admin', 'operator']), (req, res) => {
    try {
        const delRes = dbApi.data.deleteGroup(req.params.id);
...
// 更新班组
app.put('/api/groups/:id', requireRole(['admin', 'operator']), banzuUploadMiddleware, (req, res) => {
    try {
        const currentRes = dbApi.data.getAllGroups();
        ...
        const updateRes = dbApi.data.updateGroup(req.params.id, updates);
        if (!updateRes.success) throw new Error(updateRes.error || '更新班组失败');
        const refreshedRes = dbApi.data.getAllGroups();
