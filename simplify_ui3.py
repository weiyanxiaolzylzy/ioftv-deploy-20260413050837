...
# 3. Replace ALL methods from onPresetSelectChange to getTodayStr
# Find the start of old methods
...
if start_idx != -1 and end_idx != -1:
    # Get the new methods replacement
    ...
    # Replace everything from onPresetSelectChange to (but not including) getTodayStr
...
# 4. Remove the entire preset block (固定班组配置)
preset_start = 'e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-preset-block"}'
preset_end = 'e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-bundle"}'

idx1 = content.find(preset_start)
idx2 = content.find(preset_end)

if idx1 != -1 and idx2 != -1:
    # Keep the first hr and the bundle div, remove the preset block in between
    # Find the hr before preset block
    hr_before_preset = content.rfind('e("hr"', 0, idx1)
    if hr_before_preset != -1:
        # Replace from hr to just before bundle div with simple hr
        content = content[:hr_before_preset] + 'e("hr",{staticClass:"pifc-divider"}),' + content[idx2:]
        print("4. Removed preset block - OK")
    else:
        print("4. WARNING: Could not find hr before preset block")
else:
    print("4. WARNING: Could not find preset block markers")
...
# 6. Remove bundle header (班组与三方人员)
content = content.replace(
