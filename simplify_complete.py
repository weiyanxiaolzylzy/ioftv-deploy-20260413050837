...
# ====== 1. FIX PRESET BLOCK ======
# Find the preset block section to remove
# Pattern: e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-preset-block"}...[end of preset block with ],e("hr"...e("div",{staticClass:"pifc-bundle"}
# We need to remove the preset block but keep: e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-bundle"}

# Find where the hr+preset block starts
hr_preset_start = 'e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-preset-block"}'
hr_preset_idx = content.find(hr_preset_start)

# Find where the bundle div starts
bundle_start = 'e("div",{staticClass:"pifc-bundle"}'
bundle_idx = content.find(bundle_start, hr_preset_idx)

if hr_preset_idx != -1 and bundle_idx != -1:
    # Find the end of preset block - it ends with ]), before the next e("hr")
    # Search between hr_preset_idx and bundle_idx for the closing of preset block
    preset_end_search = content[hr_preset_idx:bundle_idx]
    # The preset block ends with ]),e("hr"
    preset_block_close = preset_end_search.find(']),e("hr"')
    if preset_block_close != -1:
        # Content to remove starts at hr_preset_idx and ends at hr_preset_idx + preset_block_close + 2 (the ]))
        remove_start = hr_preset_idx
        remove_end = hr_preset_idx + preset_block_close + 2
        print(f"Removing preset block: {remove_start} to {remove_end}")
        content = content[:remove_start] + 'e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-bundle"}' + content[remove_end:]
        print("1. Removed preset block - OK")
    else:
        print("1. ERROR: Could not find preset block end")
else:
    print("1. ERROR: Could not find preset block markers")
...
print("3. Updated mounted() - OK")

# ====== 4. REPLACE METHODS ======
# Find and replace methods from onPresetSelectChange to getTodayStr
...
if start_idx != -1 and end_idx != -1:
    ...
    print("4. Replaced methods - OK")
else:
    print("4. ERROR: Could not find method markers")

# ====== 5. UPDATE TEAM INPUT TO SELECT ======
...
if old_team_input in content:
    ...
    print("5. Updated team input - OK")
else:
    print("5. WARNING: Could not find team input")

# ====== 6. REMOVE BUNDLE HEADER ======
content = content.replace(
