...
# Find preset block start after methods replacement
...
if idx_preset_start != -1:
    ...
    if idx_bundle != -1:
        # Find the closing ]) of preset block by looking for the last ]), before bundle div
        search_area = content[idx_preset_start:idx_bundle]
        last_close = search_area.rfind(']),')
        
        if last_close != -1:
            preset_end = idx_preset_start + last_close + 2
            print(f"Preset block: {idx_preset_start} to {preset_end}")
            
            # Remove preset block content
        ...
        print("4. ERROR: Could not find bundle div")
        sys.exit(1)
else:
