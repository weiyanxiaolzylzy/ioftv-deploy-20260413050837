...
# Pattern: starts with e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-preset-block"}
# ends with ]),e("hr",{staticClass:"pifc-divider"}),e("div",{staticClass:"pifc-bundle"}
...
if idx_preset != -1 and idx_bundle != -1:
    ...
    # Look for ]),e("hr" pattern in the range
    search_area = content[idx_preset:idx_bundle]
    preset_close = search_area.find(']),e("hr"')
    
    if preset_close != -1:
        # Remove from idx_preset to idx_preset + preset_close + 2 (the ]))
        remove_end = idx_preset + preset_close + 2
        content = content[:idx_preset] + bundle_marker + content[remove_end:]
        print("4. Removed preset block - OK")
    else:
        print("4. ERROR: Could not find preset close")
        sys.exit(1)
else:
    print("4. ERROR: Could not find preset markers")
    sys.exit(1)
