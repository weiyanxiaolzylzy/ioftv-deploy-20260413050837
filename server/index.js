...
const parseIfcProducts = async (filePath, limit = 8000) => {
    ...
    // 获取所有 IFCRELDEFINESBYPROPERTIES，建立 expressID -> 属性数组 的映射
    ...
    const relDefinesMap = {}; // expressID -> [{ name, value }, ...]
    ...
    for (let i = 0; i < relDefinesSize; i++) {
        ...
        try {
            ...
            // RelatingPropertyDefinition 中直接包含 HasProperties，不需要再查 IFCPROPERTYSET
            ...
            for (let j = 0; j < propSize; j++) {
                ...
                if (!prop) continue;
                // IFCPROPERTYSINGLEVALUE: Name 和 NominalValue 直接在对象中
                const propName = prop.Name && prop.Name.value ? String(prop.Name.value) : '';
                let propValue = '';
                if (prop.NominalValue) {
                    // IFCLABEL 类型: value 在 _internalValue 或直接 value 中
            ...
            for (let j = 0; j < relSize; j++) {
                ...
                if (!ent) continue;
                const entId = ent.expressID;
                const numId = Number(entId);
                if (!isNaN(numId) && propList.length > 0) {
...
app.post('/api/upload-ifc', requireRole(['admin', 'operator']), (req, res) => {
    ifcUpload.single('ifc')(req, res, (err) => {
        ...
        (async () => {
            let parse = null;
            try {
                parse = await parseIfcProducts(fullPath);
            } catch (e) {
                parse = { success: false, message: e && e.message ? String(e.message) : 'IFC 解析失败' };
            }
            res.json({
                success: true,
                url,
